...
Disallow bare await deps.*() in workflow callbacks. Wrap them in step() or call them through steps.*().
...
- **ESLint rule:** `awaitly/step-no-deps-bypass`
...
Canonical awaitly slug: workflow-unreadable-condition
...
Canonical awaitly slug: workflow-unreadable-iterable
...
- **ESLint rule:** `awaitly/error-require-discriminant`
...
Raise maxIterations, bound the collection before the loop, or pass onMaxIterations: 'stop' to truncate on purpose.

- **Runtime error:** `IterationLimitError` (`error.code === 'runtime-iteration-limit'`)

### `runtime-rate-limit` \{#runtime-rate-limit\}

Wait retryAfterMs before retrying, or apply singleflight to deduplicate calls.
