...
Requires a string literal as the first argument to `step()` and step helpers (`step.sleep`, `step.retry`, `step.parallel`, `step.race`, etc.). Ensures stable step IDs for visualization and static analysis.
...
createWorkflow('workflow', deps)(async ({ step, deps }) => err({ type: 'FAILED' }));
...
createWorkflow('workflow', deps)(async ({ step, deps }) => value);
```
