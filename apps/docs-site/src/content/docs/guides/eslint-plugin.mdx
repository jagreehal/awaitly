...
Workflow callbacks should destructure their context. Only `step` is required; `raw` and `ctx` are optional depending on the entry point. In deps-first `run(deps, (s, context) => ...)`, the first parameter is the bound steps object, so the rule checks the second parameter, if you take one.
