...
#### `awaitly/workflow-prefer-step-if`

**Severity**: warn (recommended) · error (recommended-strict)

A raw `if/else` that contains steps has no stable branch id, so the static diagram can't label it deterministically. Use `step.if` for a labelled branch, or `when`/`unless` to guard a single step.

```typescript
// Bad — the branch has no stable identity in the diagram
if (user.premium) {
  await step('sendPremiumEmail', () => deps.sendEmail(user));
}

// Good — labelled, analyzable branch
await when(() => user.premium, () =>
  step('sendPremiumEmail', () => deps.sendEmail(user))
);
```

#### `awaitly/workflow-prefer-step-foreach`

**Severity**: warn (recommended) · error (recommended-strict)

A native loop (or `.forEach`/`.map`) that contains steps produces unstable, unbounded iteration ids. Use `step.forEach` so each iteration has a structured, statically-analyzable id.

```typescript
// Bad — iteration ids are unstable / unbounded
for (const item of items) {
  await step('processItem', () => deps.process(item));
}

// Good — structured iteration
...
When matching on `result.error.type`, check `isUnexpectedError(result.error)` first to separate library bugs from typed business errors.
...
```typescript
// Bad
if (result.error.type === 'TimeoutError') { /* without checking UnexpectedError first */ }

// Good
if (isUnexpectedError(result.error)) {
  log.error('library bug', result.error.cause);
} else if (result.error.type === 'TimeoutError') {
  /* */
}
```
