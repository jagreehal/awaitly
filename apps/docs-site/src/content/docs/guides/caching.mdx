...
```typescript
// Cached (explicit key)
await step.all('fetchAll', { user: () => fetchUser('1') }, { key: 'fetch:1' });
await step.map('fetchUsers', ids, fetchUser, { key: 'users:batch' });

// Not cached (no key) — runs every time
await step.all('fetchAll', { user: () => fetchUser('1') });
await step.map('fetchUsers', ids, fetchUser);
```
