...
## step(), step.all, step.map and caching

**step()** — With `createWorkflow`, use a **getter** so the step only runs on cache miss: `step('fetchUser', () => fetchUser('1'), { key: 'user:1' })`. If you pass the promise directly, it is created at call site and caching cannot skip the work.
