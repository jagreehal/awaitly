...
2. **Or you pass it at execution time** — e.g. `workflow(args, fn, { onEvent: viz.handleEvent })`, if your workflow API supports execution options.
...
export async function processOnePayment(
  ...
  return workflow(args, async (step, deps, args) => { ... });
}
