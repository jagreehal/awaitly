...
awaitly v2 consolidates ~36 subpath exports into **four entry points**. Named imports only — no `Awaitly` namespace object.

### Entry points

| Entry | Import |
|-------|--------|
| Front door | `import { ok, err, run, retry, match, ... } from 'awaitly'` |
| Minimal bundle | `import { ok, err, map, ... } from 'awaitly/result'` |
| Production tier | `import { createWorkflow, durable, ... } from 'awaitly/workflow'` |
| Testing | `import { createWorkflowHarness, ... } from 'awaitly/testing'` |

### Import rewrite table

| Removed subpath | New import |
|-----------------|------------|
| `awaitly/result`, `awaitly/run`, `awaitly/core`, `awaitly/errors`, `awaitly/tagged-error`, `awaitly/match`, `awaitly/duration`, `awaitly/circuit-breaker`, `awaitly/ratelimit`, `awaitly/cache`, `awaitly/singleflight`, `awaitly/policies`, `awaitly/conditional`, `awaitly/slugs` | `awaitly` |
| `awaitly/durable`, `awaitly/persistence`, `awaitly/engine`, `awaitly/hitl`, `awaitly/saga`, `awaitly/streaming`, `awaitly/webhook`, `awaitly/resource`, `awaitly/batch`, `awaitly/workflow` | `awaitly/workflow` |

Persistence adapter packages (`awaitly-postgres`, `awaitly-mongo`, `awaitly-libsql`) should import persistence and durable types from `awaitly/workflow`, not `awaitly/persistence` or `awaitly/durable`.

### Removed APIs

| v1 pattern | v2 replacement |
|------------|----------------|
| `import { Awaitly } from 'awaitly'` / `Awaitly.ok()` | Named imports: `import { ok, err } from 'awaitly'` |
| `import { pipe, flow, compose } from 'awaitly'` | Plain function calls or `async/await` with `run()` |
| `await workflow(async ({ step }) => ...)` | `await workflow.run(async ({ step }) => ...)` |
| `createWorkflow(deps)` | `createWorkflow('name', deps)` — name is required |
| `result.error._tag` for matching | Prefer `result.error.type` (`._tag` is a deprecated alias) |
| Schedule combinators from `awaitly/retry` | Per-dep `retry()` / `timeout()` / `fallback()` from `awaitly` |
| `awaitly/otel`, `awaitly/fetch`, `awaitly/adapters` | Deferred to future ecosystem packages — see [OpenTelemetry](/advanced/opentelemetry/) |
| `awaitly/flow`, `awaitly/functional`, `awaitly/bind-deps`, `awaitly/resolver`, `awaitly/diagnostics`, `awaitly/reliability` | Removed — use `run()`, `withDeps()`, or plain async/await |

### Quick before/after

```typescript
// v1
import { Awaitly, pipe } from 'awaitly';
...
const result = await workflow.run(async ({ step, deps }) => {
  return await step('fetch', () => deps.fetchUser('1'));
});
```

See [Installation — import paths](/getting-started/installation/#import-paths) and [Result types — match](/foundations/result-types/#match---pattern-match) for the updated surface.
...
const result = await processOrder.run(async ({ step, deps }) => {
  const order = await step('findOrder', () => deps.findOrder(orderId));
  await step('chargeOrder', () => deps.chargeOrder(order.total));
  await step('sendConfirmation', () => deps.sendConfirmation(order.email));
  return order;
});
...
| `Effect.all([...])` | `step.all('name', { ... })` or `step.all('name', { ... })` or `allAsync([...])` | Similar; `step.all` is Effect-style alias |
...
const createPaymentWorkflow = (db: Db, provider: Provider, raw: unknown, actorEmail: string) => {
  ...
  return workflow.run(async ({ step, deps }) => {
    const input = await step('validateInput', () => deps.validateInput(raw), { key: 'validate' });

    const existing = await step('findExisting', () => deps.findExisting(db, input.idemKey), { key: 'existing' });
    if (existing) {
      return { paymentId: existing.id };
    }

    await step('acquireLock', () => deps.acquireLock(db, input.idemKey), { key: 'lock' });
    const response = await step('callProvider', () => deps.callProvider(provider, input), { key: 'provider' });
    return await step('persistSuccess', () => deps.persistSuccess(db, input, response, actorEmail), { key: 'persist' });
  });
};
...
1. **Add retries**: Use `step.retry()` for transient failures inside workflows, or per-dep `retry()` on dependencies — see [Policies](/advanced/policies/)
2. **Add timeouts**: Use `step.withTimeout()` or per-dep `timeout()` wrappers
3. **Add fallbacks**: Use per-dep `fallback()` where a default is acceptable
4. **Add caching**: Use step keys for idempotent operations
