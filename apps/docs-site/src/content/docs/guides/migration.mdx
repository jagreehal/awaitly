...
| `yield* someEffect` | `await step(fn)` | Unwrap in workflow |
| `Effect.tryPromise({ try, catch })` | `step.try(() => ..., { onError })` | Similar |
| `Effect.all([...])` | `step.parallel({ ... })` or `allAsync([...])` | Similar |
| `Schedule.exponential()` | `step.retry({ backoff: 'exponential' })` | Built-in options |
| `Effect.timeout(duration)` | `step.try(..., { timeout: '5s' })` | String durations |
...
const result = await workflow(async (step) => {
  const user = await step(fromEffect(legacyEffectService.getUser('1')));
  return user;
});
...
const workflow = createWorkflow({ getUser, getPosts });

const result = await workflow(async (step, deps) => {
  const user = await step(() => deps.getUser('1'));
  const posts = await step(() => deps.getPosts(user.id));
  return { user, posts };
});
```
...
const createPaymentWorkflow = (db: Db, provider: Provider, raw: unknown, actorEmail: string) => {
  ...
  return workflow(async (step, d) => {
    const input = await step(() => d.validateInput(raw), { key: 'validate' });

    const existing = await step(() => d.findExisting(db, input.idemKey), { key: 'existing' });
    if (existing) {
      return { paymentId: existing.id };
    }

    await step(() => d.acquireLock(db, input.idemKey), { key: 'lock' });
    const response = await step(() => d.callProvider(provider, input), { key: 'provider' });
    return await step(() => d.persistSuccess(db, input, response, actorEmail), { key: 'persist' });
  });
};
