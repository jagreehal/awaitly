...
Add retries and timeouts to individual steps without wrapping your entire workflow in try/catch. For retries **outside** workflows (plain async/Result code), use the [`retry` policy](foundations/result-types/#retry-with-the-retry-policy) from `awaitly`. For policies declared on dependencies, see [deps-level policies](#deps-level-policies) below or the [Policies guide](advanced/policies/).
  ...
  steps={[
    {
      id: 'timeout',
      label: 'timeout',
      description: 'Abort an attempt if it runs too long.',
      duration: '1.5s',
    },
    {
      id: 'retry',
      label: 'retry',
      description: 'Retry transient failures with backoff.',
      duration: '2s',
    },
    {
      id: 'succeed',
      label: 'succeed or fail',
      description: 'On exhaustion, the workflow exits with the last error.',
      duration: '1.5s',
    },
  ]}
  autoPlay={true}
...
<Aside type="tip" title="Always use an ID for timeouts">
The step ID (first argument) appears in events and timeout errors. When you see `"fetchUser timed out after 5000ms"` in logs, you know exactly what failed.
</Aside>
...
if (!result.ok && isStepTimeoutError(result.error)) {
  ...
  console.log(`${meta?.stepName} timed out after ${meta?.timeoutMs}ms`);
}
/*
Output:
...
```typescript
// Declare the tag so it joins the workflow's error union
const workflow = createWorkflow({ apiCall }, { errors: ['API_TIMEOUT'] });

const result = await workflow.run(async ({ step, deps }) => {
  return step.withTimeout('apiCall', () => deps.apiCall(), {
    ms: 5000,
    onTimeout: () => 'API_TIMEOUT',
  });
});
// result.error: 'API_TIMEOUT' | UnexpectedError | <apiCall's own errors>
```

The timeout error is checked against the workflow's error union, so a tag you
have not declared is a compile error rather than a silent `unknown`. Declare new
tags with `errors: [...]`, on the workflow or on the `step.withTimeout` call
itself. `step.retry` takes the same option.
  </TabItem>
</Tabs>
...
According to [AWS Architecture Blog on Exponential Backoff and Jitter](https://aws.amazon.com/blogs/architecture/exponential-backoff-and-jitter/), adding jitter cuts collision rates when many clients retry simultaneously.
...
By default, retry skips `UnexpectedError` and untagged throws (bugs). Typed errors still retry. Narrow further with `shouldRetry` so `NOT_FOUND` or `UNAUTHORIZED` are not retried either. Pass `retryIf: () => true` only if you want to retry throws.
</Aside>
...
const result = await workflow.run(async ({ step, deps }) => {
  ...
  // Per-attempt timeout goes in the retry options — don't nest step calls,
  // `step.withTimeout` returns the unwrapped value, not a Result.
  const user = await step.retry(
    'fetchUser',
...
| `shouldRetry` | `(error) => boolean` | typed errors only | Condition for retry. Does not retry `UnexpectedError` or untagged throws. Alias: `retryIf`. |
| `clock` | `Clock` | run/workflow clock, else `systemClock` | Time source for delays. Pass `createTestClock()` in tests. Policy `retry()` / `timeout()` need `clock` at wrap time. |

## Deps-level policies

Declare retry and timeout on dependencies so call sites stay clean and `awaitly-analyze` can read the policy chain from the deps literal:

```typescript
import { retry, timeout, tryAsync, createWorkflow } from 'awaitly';

// Policies wrap *functions*, so `tryAsync` goes inside the dep.
...
// `steps` auto-tracks each dep call; `raw` calls the dep untracked.
await checkout.run(async ({ steps }) => {
  await steps.charge(amount); // auto-tracked step; retries + timeout applied
});
```

Policies compose inside-out: `retry(timeout(fn, ms), opts)` times out each attempt, then retries on failure. See [Policies](advanced/policies/) for `fallback`, error-union rules, and legacy StepOptions bundles.

Inside a workflow body you can still use **`step.retry`** and **`step.withTimeout`** when retry or timeout is the primary verb for a one-off operation. See the sections above.

## Testing with a clock

Pass `createTestClock()` from `awaitly/testing` so retry delays, `step.sleep`, `step.withTimeout`, and circuit-breaker windows do not wait on real time. On `step.retry` / `retryAsync`, set `jitter: false` so delays are exact.

The same `clock` option exists on `run()`, `createWorkflow` / `workflow.run` (per-run overrides creation-time), `retry()`, `timeout(fn, after, { clock })`, and `createCircuitBreaker`.
...
const timed = timeout(slowOp, 500, { clock });
...
See [Testing](guides/testing/#testing-time-dependent-workflows) for harness timestamps vs control-flow time.
