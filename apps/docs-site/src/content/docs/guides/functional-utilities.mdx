...
For tagged-union pattern matching (e.g. event handlers or discriminated unions), use `awaitly/core`: `Match`, `matchTag`, `matchWhen`, `MatchTag`, and related exports. See [Pattern matching (awaitly/core)](/docs/reference/api/#pattern-matching-awaitlycore) in the API reference.
