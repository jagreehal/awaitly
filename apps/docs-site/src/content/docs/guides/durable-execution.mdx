...
See also: [Where options go](guides/persistence/#where-options-go-creation-vs-per-run) (creation vs per-run).
...
To persist across restarts or share state across processes, pass a store from `postgres()`, `mongo()`, or `libsql()`:
...
3. **Resume**: Skip completed steps using cached results (crashed steps are retried — see [Resuming failed steps](#resuming-failed-steps))
...
The `store` option accepts **DurableStore**, the wider contract the shipped adapters implement (their `save` also takes a `ResumeState` and their `load` returns one). Both types are exported from `awaitly/durable`, so `postgres()`, `mongo()`, and `libsql()` can be passed straight to `durable.run` — no cast needed.

**Note:** Snapshot stores hold workflow checkpoints. For streaming output inside a step (AI tokens, etc.), see [Streaming](foundations/streaming/).
...
A step can fail two ways, and a resume treats them differently:

- It **threw** — the worker died, the socket dropped, the process was killed. That is a crash, not a verdict, so the step is retried on resume.
- It returned a typed **`err`** — `DECLINED`, `VALIDATION_FAILED`. That is a decision the step reached, so it stays decided and is not re-run.

`resumeFailedSteps` controls this:

```typescript
const result = await durable.run(deps, workflowFn, {
  id: 'checkout-123',
  store,
  resumeFailedSteps: 'crashed', // default: retry throws, keep typed errors
  // 'all' — restore every failed step, including crashes
});
```

## Checkpoints inside `step.forEach`

A step written inside a `step.forEach` callback runs once per item, and each iteration gets its own checkpoint. Resume skips the items that already completed and picks up at the first one that did not:

```typescript
await step.forEach('charges', invoices, {
  stepIdPattern: 'charge-{i}',
  run: async (invoice) => {
    // Keyed per iteration: charge@charge-0, charge@charge-1, ...
    await step('charge', () => deps.charge(invoice));
  },
});
```

`stepIdPattern` names the iteration for both the diagram and the checkpoint, so the two agree. Nested loops concatenate their iteration names, and the scope is per run, so concurrent runs never share checkpoints. The same applies to `step.try`, `step.fromResult`, `step.withFallback`, and `step.withResource`.

`step.retry` and `step.withTimeout` checkpoint by id (and by iteration) too, so a retried step is skipped on resume rather than run again.

## Declaring Extra Errors

A durable run infers its error union from its deps, the same as `createWorkflow`. An error introduced by `step.try`, or a system error you want in the static union, is declared with `errors`:
...
const result = await durable.run(
  ...
  async ({ step, deps }) => {
    ...
    // type-checks because 'PARSE_FAILED' is declared below
...
No `as const`, the option is a `const` type parameter. Omitting `errors` leaves the union exactly as the deps infer it.

This is also how a stream failure gets into the static union. `STREAM_READ_ERROR` already arrives as a typed value rather than an `UnexpectedError`, but declaring it lets a boundary `switch` stay exhaustive:
...
- **Step outputs** – You changed what a step returns in a way that later steps or the workflow logic no longer accept (e.g. type or shape change). Old cached results would be invalid, and nothing else can detect that for you.

If you only change non-durable logic (e.g. logging, non-keyed steps, or code after the last keyed step), you do **not** need to bump.

### Reordering and renaming are caught for you

Changing step names, keys, or order used to require remembering to bump `version` , 
forget, and a resumed run could read one step's checkpoint into a different step.
That is no longer a matter of discipline.

Each snapshot records the order its steps executed in. On resume, awaitly compares
the replayed order against that record **before each step reads its stored value**,
and rejects a drifted resume with a `WorkflowShapeDriftError` naming the position and
both keys:

```text
Workflow 'order-123' changed shape since its checkpoint was written:
step 2 was 'authorizePayment' and is now 'submitReservation'.
Resuming would read the wrong checkpoint.
```

The check runs in the `onBeforeStep` hook, which fires ahead of the cache and snapshot
lookup, so the mismatched value is never read, the run fails cleanly, and the original
checkpoint is left untouched for you to migrate or clear.

This matters most for repeated calls to the same dependency. Their step keys are
position-derived (`getUser`, `getUser#2`, …), so the key alone cannot tell
`getUser("a")` from `getUser("b")`, swapping two such calls leaves the key sequence
completely unchanged while the checkpoints now belong to different arguments.

To catch that, the snapshot also records a fingerprint of each bound step's arguments
and compares it alongside the key:

```ts
// v1 — checkpointed
await steps.getUser("a");   // getUser
await steps.getUser("b");   // getUser#2

// v2 — same keys, different arguments. Rejected on resume.
await steps.getUser("b");   // getUser  <- would have read "a"'s result
await steps.getUser("a");   // getUser#2
```

Arguments that cannot be fingerprinted (circular structures, values JSON drops) record
no fingerprint, and a missing fingerprint is treated as *no information* rather than a
match, so the check never produces a false rejection, but it also cannot detect this
particular edit for those steps. Bump `version` if you reorder calls whose arguments
aren't serializable.
...
const result = await durable.run(
  deps,
  async ({ step, deps }) => {
    const user = await step('fetchUser', () => deps.fetchUser(id), { key: 'fetch-user' });
    const order = await step('createOrder', () => deps.createOrder(user), { key: 'create-order' });
    await step('processPayment', () => deps.processPayment(order), { key: 'payment' }); // May get cancelled here
    return order;
  },
  {
    id: 'order-123',
...
**Listing pending work:** Do not load the world into memory. `durable.listPending(store, options)` takes `{ prefix?, limit? }` and returns `Array<{ id, updatedAt }>`. It delegates straight to `store.list(options)`. Example: `const pending = await durable.listPending(store, { limit: 50 });`.

There is no offset/cursor pagination or ordering in the core API. To page or order beyond `prefix` + `limit`, query your store directly, the adapters expose their underlying client, so use `ORDER BY updated_at ... LIMIT ... OFFSET ...` (Postgres/LibSQL) or a `sort`/`skip` cursor (Mongo) and feed the resulting ids to `durable.run`.

**Bulk delete:** Use `durable.deleteStates(store, ids, { concurrency?, continueOnError? })` for admin/cleanup. It loops over `store.delete(id)` in batches bounded by `concurrency` (default 10). Returns `{ deleted }`, or `{ deleted, errors }` when `continueOnError` is true and any delete failed.
...
See also: [Persistence](guides/persistence) and the store adapters above (Postgres, Mongo).
