...
| Flag | Default | Description |
|------|---------|-------------|
| `--types` / `--no-types` | on | Generate `.types.ts` (input, output, and the error union) |
...
3. `run@file:line`, for a genuinely anonymous call.

Generated `.types.ts` filenames follow the workflow name, so a committed one may be written under a new name after a rename.
