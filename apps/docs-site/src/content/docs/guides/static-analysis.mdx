...
Native `if`/`else`, `for...of`, and `while` are analyzed too: the analyzer derives a stable branch id from the condition and a stable loop id from the iterable, so plain TypeScript produces deterministic diagrams without any DSL. Run `--assert-diagrammable` in CI — it flags only control flow whose condition or iterable cannot be read statically (a call result, a computed member), which you fix by hoisting it into a named variable.
