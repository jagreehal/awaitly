...
The analyzer also supports workflows defined by **extending `Workflow`** from `awaitly/workflow`:

```typescript
import { Workflow } from 'awaitly/workflow';

class GetUserWorkflow extends Workflow<typeof deps> {
  constructor() {
    super('getUser', deps);
  }
...
- **Workflow name** is taken from the first argument to `super("name", deps)` (e.g. `"getUser"`, `"image-processing"`).
- **Steps** are extracted from the `run(event, step)` body; use `step('id', fn)` so step IDs appear in diagrams and IR.
- When analyzing **files** (e.g. `analyze('./workflows.ts')`), ensure the file imports `Workflow` from `awaitly/workflow` so the analyzer can resolve it.
...
- **StaticWorkflowNode** (root): `workflowName`, `source`, `dependencies`, `children`, `description`, `markdown`, `jsdocDescription?`, `errorTypes`.
  `description` and `markdown` are set for `createWorkflow` / `createSagaWorkflow` (from options or deps) and for **class-based workflows** from the constructor options (third argument to `super("name", deps, options)`). They are undefined for `run()` / `runSaga()`. JSDoc above the workflow variable or class is extracted as `jsdocDescription` when present.
...
For **class-based workflows**, the name used with `.named(name)` is the string passed to `super("name", deps)` (e.g. `.named('getUser')`), not the class name.
