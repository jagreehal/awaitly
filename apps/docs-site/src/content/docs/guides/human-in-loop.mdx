...
const result = await refundWorkflow.run(async ({ step }) => {
...
if (!result.ok && isPendingApproval(result.error)) {
  ...
  await store.save(orderId, collector.getResumeState());
}
```
...
const savedState = await store.load(orderId);
...
const result = await workflow.run(async ({ step, deps }) => {
  ...
  const approval = await step('requireApproval', deps.requireApproval, { key: 'approve:refund' });
...
```typescript
import { isApprovalRejected } from 'awaitly/durable';
...
// The workflow is supplied as a *factory*. It must pass `resumeState` and
// `onEvent` straight through to createWorkflow — HITL tracking depends on both.
// Deps flow through, so `raw` and `step` are fully typed inside workflowFn.
const workflowFactory = ({ resumeState, onEvent }: HITLWorkflowFactoryOptions) =>
  createWorkflow('workflow', deps, { resumeState, onEvent });

const workflowFn = async ({ step, deps }) => {
  const data = await step('fetchData', () => deps.fetchData());
  // The step id must match the approval key.
  await step('approve:data', () => deps.requireApproval());
  return data;
};

// Execute. Returns a status-tagged union, not a Result.
const execution = await orchestrator.execute(
  'data-workflow',
  workflowFactory,
  workflowFn,
  { userId: 'u-1' },          // input, available as `args`
  { runId: 'wf-1' }
);

if (execution.status === 'paused') {
  // Approve — this auto-resumes any waiting workflow by default.
  await orchestrator.grantApproval('approve:data', { ok: true }, {
    approvedBy: 'alice',
  });

  // Or drive the resume yourself:
...
```typescript
import { createApprovalWebhookHandler, createMemoryApprovalStore } from 'awaitly/durable';
...
const result = await expenseWorkflow.run(async ({ step, deps }) => {
  const expense = await step('validateExpense', () => deps.validateExpense(data));
  const approval = await step('requireManagerApproval', deps.requireManagerApproval, { key: 'manager-approval' });
  return await step('processPayment', () => deps.processPayment(expense, approval));
});

if (!result.ok && isPendingApproval(result.error)) {
  await store.save('expense-123', collector.getResumeState());
...
```typescript
import { createHITLOrchestrator, NotificationChannel } from 'awaitly/durable';
...
```typescript
import { gatedStep, isPendingApproval } from 'awaitly/durable';
...
const result = await workflow.run(async ({ step, deps }) => {
  ...
  await step(
    'gatedSendEmail',
    () => deps.gatedSendEmail({ to: 'external@other.com', subject: 'Hello', body: '...' }),
    { key: 'send-email' }
  );
});
...
const result = await expenseWorkflow.run(async ({ step }) => {
...
// Show approval chain status (`pending` is a string[] of step keys)
const stages = ['manager', 'finance', 'ceo'];
const completed = stages.filter(s => !pending.some(key => key.includes(s)));
...
// The workflow factory is passed per call, not in the constructor.
const workflowFactory = ({ resumeState, onEvent }: HITLWorkflowFactoryOptions) =>
  createWorkflow('workflow', deps, { resumeState, onEvent });
...
async function recoverWorkflows() {
  ...
  for (const workflow of incomplete) {
    ...
    try {
      ...
      if (anyResolved) {
        // Resume the workflow — resume() takes the factory and the workflow fn.
        const execution = await orchestrator.resume(
          workflow.id,
          workflowFactory,
          workflowFn
        );
        console.log(`Workflow ${workflow.id}:`, execution.status);
      }
    } catch (error) {
