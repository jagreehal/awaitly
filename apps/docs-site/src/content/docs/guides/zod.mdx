...
const result = await workflow(async (step, deps) => {
  const user = await step(() => deps.validate({ email: 'test@example.com', age: 25 }));
...
const createUser = async (rawInput: unknown) => {
  return createUserWorkflow(async (step, d) => {
    ...
    const user = await step(() => d.saveToDatabase(input));

    await step(() => d.sendWelcomeEmail(user.email));
...
export const POST = async (request: Request) => {
  ...
  const result = await workflow(async (step, d) => {
    const input = await step(() => d.validatePost(body));
    const post = await step(() => d.createPost(input));
    return post;
  });
...
const register = async (rawInput: unknown) => {
  return registerWorkflow(async (step, d) => {
    ...
    const emailExists = await step(() => d.checkEmailExists(input.email));
    if (emailExists) {
      return await step(err({ type: 'EMAIL_TAKEN' as const, email: input.email }));
    }

    // Step 3: Create user
    const user = await step(() => d.createUser(input));
...
- [Workflows](/concepts/workflows/) for composing validated operations
