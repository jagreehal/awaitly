...
- **Type-safe validation errors**: Know exactly what went wrong, not only "validation failed"
...
const deps = {
  validateInput: (deps: unknown) => Promise.resolve(zodToResult(CreateUserSchema, raw)),
  saveToDatabase,
  sendWelcomeEmail,
};
...
const deps = {
  validatePost: (deps: unknown) => Promise.resolve(zodToResult(CreatePostSchema, raw)),
  createPost,
};
...
```typescript
import { z } from 'zod';
import { ok, err, type Result, type AsyncResult, createWorkflow } from 'awaitly';
...
const deps = {
  ...
  // Declaring the failure as a dep puts EMAIL_TAKEN in the inferred error
  // union — an `err()` returned inline from a step would not be.
  emailTaken: async (email: string): AsyncResult<never, EmailTakenError> =>
    err({ type: 'EMAIL_TAKEN', email }),
};
...
const register = async (rawInput: unknown) => {
  return registerWorkflow.run(async ({ step, deps }) => {
    ...
    if (emailExists) {
      return await step('emailTaken', () => deps.emailTaken(input.email));
    }
...
2. **Use Zod's `path`**. It tells you exactly which field failed
