  ...
  return {
    searchDocs: async (query: string, toolCallId?: string) => {
      ...
      return step(
        ...
        { key, description: 'Search documents' }
      );
    },
...
const result = await chatWorkflow.run(async ({ step, deps }) => {
  ...
  while (iterations++ < maxIterations) {
    const response = await step(
      'chat',
      () => deps.chat(messages),
      { key: `chat:${iterations}`, description: `Chat iteration ${iterations}` }
    );
...
const result = await aiWorkflow.run(async ({ step, deps }) => {
  const response = await step(
    ...
    { key: 'explain', description: 'Generate explanation' }
  );
