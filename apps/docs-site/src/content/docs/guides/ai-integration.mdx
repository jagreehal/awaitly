...
const result = await workflow(async (step) => {
  ...
  const stream = await step('generateStream', () => generateStream(prompt));
...
const result = await workflow(async (step) => {
  ...
  while (true) {
    const response = await step('chat', () => chat(messages), {
...
// Execute workflow
const result = await chatWorkflow(async (step) => {
...
const aiWorkflow = createWorkflow({
  generateText: async (prompt: string): AsyncResult<string, 'AI_ERROR'> => {
    try {
