...
function CheckoutButton({ cartId }: { cartId: string }) {
  const { status, data, error, run } = useWorkflow(
    ...
    async (step) => {
      const cart = await step(validateCart(cartId));
      const payment = await step(processPayment(cart.total));
      await step(sendConfirmation(cart.email, payment.id));
...
export async function checkout(
  ...
  return await workflow(async (step) => {
    const cart = await step(validateCart(cartId));
    const payment = await step(processPayment(cart.total));
    await step(sendEmail(cart.email, payment.receiptUrl));
    return { orderId: payment.id };
  });
}
```
...
export async function POST(request: Request) {
  ...
  const result = await workflow(async (step) => {
    const validated = await step(validateOrder(body));
    const charge = await step(chargeCard(validated.amount));
    const order = await step(createOrder(validated, charge.id));
    return order;
  });
...
app.post('/api/orders', resultHandler(async (req) => {
  ...
  return await workflow(async (step) => {
    const order = await step(validateOrder(req.body));
    const payment = await step(processPayment(order.total));
    return { orderId: payment.id };
  });
}));
```
...
router.post('/users', async (req, res) => {
  ...
  const result = await workflow(async (step) => {
    const validated = await step(validateUser(req.body));
    const user = await step(createUser(validated));
...
fastify.post('/orders', async (request, reply) => {
  const result = await request.workflow(
    ...
    async (step) => {
      const order = await step(validateOrder(request.body));
      const payment = await step(processPayment(order.total));
      return { orderId: payment.id };
    }
  );
...
fastify.post('/orders', {
  ...
  const result = await createWorkflow({ processOrder })(async (step) => {
    return await step(processOrder(request.body));
  });
...
const result = await workflow(
  async (step, deps, ctx) => {
    const user = await step(fetchUser(ctx.userId));
    await step(logAction(ctx.requestId, 'user_fetched'));
    return user;
  },
  {
    context: {
