...
export function useWorkflow<T, E, Deps extends Record<string, unknown>>(
  ...
  const run = useCallback(async () => {
    ...
    const result = await workflow(workflowFn);
...
function CheckoutButton({ cartId }: { cartId: string }) {
  const { status, data, error, run } = useWorkflow(
    ...
    async (step) => {
      const cart = await step('validateCart', () => validateCart(cartId));
      const payment = await step('processPayment', () => processPayment(cart.total));
      await step('sendConfirmation', () => sendConfirmation(cart.email, payment.id));
...
export async function checkout(
  ...
  return await workflow(async (step) => {
    const cart = await step('validateCart', () => validateCart(cartId));
    const payment = await step('processPayment', () => processPayment(cart.total));
    await step('sendEmail', () => sendEmail(cart.email, payment.receiptUrl));
    return { orderId: payment.id };
  });
}
```
...
export async function POST(request: Request) {
  ...
  const workflow = createWorkflow({ validateOrder, chargeCard, createOrder });
  const result = await workflow(async (step) => {
    const validated = await step('validateOrder', () => validateOrder(body));
    const charge = await step('chargeCard', () => chargeCard(validated.amount));
    const order = await step('createOrder', () => createOrder(validated, charge.id));
    return order;
  });
...
app.post('/api/orders', resultHandler(async (req) => {
  const workflow = createWorkflow({ validateOrder, processPayment });
  return await workflow(async (step) => {
    const order = await step('validateOrder', () => validateOrder(req.body));
    const payment = await step('processPayment', () => processPayment(order.total));
    return { orderId: payment.id };
  });
}));
```
...
router.post('/users', async (req, res) => {
  const workflow = createWorkflow({ validateUser, createUser, sendWelcome });

  const result = await workflow(async (step) => {
    const validated = await step('validateUser', () => validateUser(req.body));
    const user = await step('createUser', () => createUser(validated));
...
fastify.post('/orders', async (request, reply) => {
  const result = await request.workflow(
    ...
    async (step) => {
      const order = await step('validateOrder', () => validateOrder(request.body));
      const payment = await step('processPayment', () => processPayment(order.total));
      return { orderId: payment.id };
    }
  );
...
fastify.post('/orders', {
  ...
  const result = await createWorkflow({ processOrder })(async (step) => {
    return await step('processOrder', () => processOrder(request.body));
  });
...
```typescript
const workflow = createWorkflow({ fetchUser, logAction });

const result = await workflow(
  async (step, deps, ctx) => {
    const user = await step('fetchUser', () => fetchUser(ctx.userId));
    await step('logAction', () => logAction(ctx.requestId, 'user_fetched'));
    return user;
  },
  {
    context: {
