...
const workflow = createWorkflow('workflow', { validateInput: (deps: unknown) => zodToResult(CreateUserSchema, raw),
...
const transferFunds = createSagaWorkflow('saga', { debitAccount: async (accountId: string, amount: number) => {
...
const result = await transferFunds.run(async ({ step, deps }) => {
  ...
  // Record transaction
  await step('createTransaction', () => deps.createTransaction({
    sourceId,
    destId,
    amount,
    timestamp: new Date(),
...
const signUp = async (rawInput: unknown) => {
  return signUpWorkflow.run(async ({ step, deps }) => {
    ...
    const user = await step('createUser', () => deps.createUser({
      email: input.email,
      passwordHash,
      name: input.name,
    }));
...
export const POST = async (request: Request) => {
  ...
  if (!result.ok) {
    switch (result.error.type) {
      ...
      default:
        // The union also carries UnexpectedError — handle it so the block
        // always returns and `result.value` narrows below.
