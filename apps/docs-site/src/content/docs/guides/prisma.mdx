...
const workflow = createWorkflow('workflow', { validateInput: (raw: unknown) => zodToResult(CreateUserSchema, raw),
...
const result = await transferFunds(async (ctx, deps) => {
  ...
  // Record transaction
  await ctx.step('createTransaction', () => deps.createTransaction({
    sourceId,
    destId,
    amount,
    timestamp: new Date(),
...
const signUp = async (rawInput: unknown) => {
  return signUpWorkflow(async (step, d) => {
    ...
    const passwordHash = await step('hashPassword', () => d.hashPassword(input.password));
    const user = await step('createUser', () => d.createUser({
      email: input.email,
      passwordHash,
      name: input.name,
    }));
