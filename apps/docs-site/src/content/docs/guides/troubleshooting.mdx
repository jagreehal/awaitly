...
### My error type is wider than I expected

`err()` takes a `const` type parameter, so error literals keep their literal
type on their own, no `as const` required:
...
// error type is 'NOT_FOUND', not string
```

Annotating the return type is still worth doing when you want one named error
union that the compiler checks every branch against:
...
Annotating also keeps inferred error *objects* free of `readonly` modifiers,
which matters if an error carries an array you later assign to a mutable type.
...
### Read the stack trace on `cause`

awaitly reports an `UnexpectedError` when a dependency throws. The original error and its stack frames remain on `cause`:

```typescript
async function readConfig(path: string) {
  JSON.parse(await fs.readFile(path, 'utf8'));   // throws
  return ok(path);
}

const result = await run({ readConfig }, async (s) => s.readConfig('/etc/app'));

if (!result.ok) {
  console.log(result.error.type);        // 'UnexpectedError'
  console.log(result.error.cause.stack);
}
```
...
SyntaxError: Expected property name or '}' in JSON at position 2
    ...
    at readConfig (/app/src/config.ts:4:8)
    at .../awaitly/dist/index.js:911:31
```

The top frames identify the line in your code that threw. awaitly runs steps with `await`, so the JavaScript engine builds the same stack it builds for other async functions. Your debugger can step through those frames.

Log `result.error.cause` for failures you did not model. Inspect `result.error.type` to identify the workflow failure and `cause.stack` to find the throw site.
...
2. **Library workflows**: If you call a function like `processOnePayment(..., deps)` that creates the workflow internally, that function must accept optional workflow options (e.g. `{ onEvent }`) and forward them to `createWorkflow`. Otherwise your visualizer never receives events. See [Visualization: Library workflows](guides/visualization/#library-workflows).
3. **Steps run**: The workflow must execute at least one `step(...)` so that `step_start` / `step_complete` events are emitted. If the workflow exits early or throws before any step, you may only see `workflow_start`.
