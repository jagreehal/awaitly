...
See [Why thunks?](/foundations/thunks/) for the mental model and the “why `step('id', somePromise)` may work” nuance.
...
<Aside type="note" title="Caching and run / all / map">
In `createWorkflow`, use a getter with `step.run` when you use a key so cache hits don't run the operation. For `step.all` and `step.map`, caching applies only when you pass `{ key: '...' }`; without a key they never cache by step id.
</Aside>
...
Run a child workflow as a step. First argument is the step ID; second is a getter that returns the child workflow run (e.g. `() => childWorkflow.run(...)`). Optional `key`. Events and cache go through the step engine; [awaitly-analyze](/docs/guides/static-analysis/) detects child workflow refs when the getter calls `childWorkflow.run(...)` and draws them as workflow-ref nodes.
