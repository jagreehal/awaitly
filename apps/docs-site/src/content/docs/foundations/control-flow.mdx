...
<Aside type="tip" title="Diagrammability">
Raw `if/else`, native loops, and computed step ids create gaps in static diagrams. Use `step.if`, `when`/`unless`, and `step.forEach` so [`awaitly-analyze`](/guides/static-analysis/) can produce deterministic diagrams and `--assert-diagrammable` passes in CI. The ESLint rules [`workflow-prefer-step-if`](/guides/eslint-plugin/#awaitlyworkflow-prefer-step-if) and [`workflow-prefer-step-foreach`](/guides/eslint-plugin/#awaitlyworkflow-prefer-step-foreach) flag raw control flow as you write it.
</Aside>
