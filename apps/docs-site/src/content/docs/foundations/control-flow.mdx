...
Give parallel groups a name for visualization using `step.all(name, ...)` (Effect-style) or `step.all(name, ...)`:
...
// Or step.all array form with allAsync
const result = await workflow.run(async ({ step, deps }) => {
  const [user, posts] = await step.all('Fetch user data', () =>
...
### step.if for labelled branches

When you need `if/else` with steps in both branches, use `step.if` so the analyzer can label the decision node deterministically:

```typescript
// ✅ Labelled branch — static diagram shows the decision id
...
// ❌ Raw if/else — branch has no stable identity in the diagram
if (user.isPremium) {
  await step('sendPremiumEmail', () => deps.sendPremiumEmail(user));
}
```

For single guarded steps (no else branch), prefer [`when`](guides/conditional-execution/) or [`unless`](guides/conditional-execution/) — they emit skip events and integrate with the visualizer.

`step.if` emits a `decision` event on each evaluation. The visualizer and `awaitly-analyze` consume it automatically; no `trackIf` wiring needed. See [Visualization →](guides/visualization/#decision-tracking).

<Aside type="tip" title="Native `if` and `for` are fully diagrammable">
You do **not** need `step.if` or `step.forEach` to get a deterministic diagram.
`awaitly-analyze` reads plain `if`/`else`, `for...of`, and `while` directly, deriving a
stable branch id from the condition (`user.isPremium` → `user-is-premium`) and a stable
loop id from the iterable (`order.items` → `order-items`). Write ordinary TypeScript:

```typescript
if (user.isPremium) {
  await steps.applyDiscount(order);
}

for (const item of order.items) {
  await steps.reserve(item);
}
```

The one thing that defeats it is a condition or iterable the analyzer can't read from
the source — a call result (`if (checkEligibility(user))`) or a computed member
(`items[i].ready`). Hoist those into a named variable and the diagram comes back:

```typescript
const isEligible = checkEligibility(user);   // now readable
if (isEligible) { /* ... */ }
```

`--assert-diagrammable` flags only those genuinely unreadable cases. Reach for
`step.if` / `step.forEach` when you want their **runtime** features — skip events,
bounded iteration, per-iteration keys — not to satisfy the analyzer.
</Aside>
...
```typescript
import { allAsync, partition, ok, err, type AsyncResult, createWorkflow } from 'awaitly';
