...
const result = await workflow(async (step) => {
  const user = await step(fetchUser('1'));        // First
  const posts = await step(fetchPosts(user.id));  // Second (after user)
  const comments = await step(fetchComments(posts[0].id)); // Third (after posts)
...
const result = await workflow(async (step) => {
  const [user, notifications, settings] = await step(() =>
    allAsync([
      fetchUser('1'),
