...
Time source for retry delays, `step.sleep`, and `step.withTimeout`. Defaults to the system clock. Per-run `clock` overrides creation-time.

```typescript
import { createTestClock } from 'awaitly/testing';

const clock = createTestClock();
const workflow = createWorkflow('workflow', deps, { clock });
...
Policy wrappers (`retry()`, `timeout()`) do not pick up this clock automatically. Pass `clock` at wrap time, or use `step.retry` / `step.withTimeout` / `step.sleep` inside the run. See [Retries & Timeouts](guides/retries-timeouts/#testing-with-a-clock).
...
| `clock` | Clock for retry delays, `step.sleep`, and `step.withTimeout` |
...
Workflows are functions. Compose them naturally:
