...
<Tabs>
  <TabItem label="With ErrorsOf (best DX)">
```typescript
...
Use `ErrorOf<typeof fn>` for a single dep or `Errors<[typeof fn1, typeof fn2]>` for multiple deps to derive the error union. Your step errors are preserved in the result as `E | UnexpectedError`. Use `catchUnexpected` only when you want a custom unexpected type instead of the default.
</Aside>
...
await checkout.run(async ({ step, deps }) => { /* different flow */ });
```
  </TabItem>
