...
By default, workflows include `UnexpectedError` in the error union. Use strict mode for closed error unions where you explicitly declare all possible errors:

<Tabs>
  <TabItem label="Default (open union)">
```typescript
...
```
  ...
  <TabItem label="Strict (closed union)">
```typescript
...
With strict mode:
- TypeScript will error if a dep can produce an undeclared error
- `UnexpectedError` is replaced with your custom error shape
- You have full control over the error union

<Aside type="tip" title="When to use strict mode">
Use strict mode when you need a closed error union for exhaustive switch statements, or when you want compile-time verification that all error types are declared.
</Aside>
