...
You can also define workflows by **extending the `Workflow` class** from `awaitly/workflow` and implementing `run(event, step)`; run them with `instance.execute(payload)`. The same step API applies. All workflow options (cache, resumeState, snapshot, onEvent, signal, description, markdown, etc.) are passed to the **constructor** as the third argument: `new MyWorkflow('name', deps, options)`. **`execute(payload, options?)`** only accepts the run input and optional `{ signal, instanceId }`. To resume, construct with `{ resumeState }` or `{ snapshot }`, then call `execute(payload)`. See the [API reference](/docs/reference/api/) for the Workflow class.
...
Workflow error unions are always closed. By default, thrown exceptions become `UnexpectedError`. To use a custom type for unexpected errors (e.g. for exhaustive switches), pass `catchUnexpected`:

<Tabs>
  <TabItem label="Default (UnexpectedError)">
```typescript
// Default - error union is E | UnexpectedError
const workflow = createWorkflow('workflow', deps);
...
```typescript
// Custom unexpected - closed union with your shape
const workflow = createWorkflow('workflow', deps, {
...
- The error union remains closed; your custom type replaces `UnexpectedError`.
...
Use `catchUnexpected` when you need a closed union with a custom unexpected shape (e.g. for exhaustive handling or consistent error reporting).
</Aside>
