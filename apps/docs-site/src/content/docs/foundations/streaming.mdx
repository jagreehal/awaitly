...
<Aside type="note" title="Different from workflow checkpoints">
Streaming stores **ordered items inside a step** (tokens, chunks). For **workflow-level resume** after restart, see [Persistence](guides/persistence/) and [Durable Execution](guides/durable-execution/).
</Aside>
...
```
  ...
  <TabItem label="File (On-disk JSONL)">
```typescript
...
```typescript
import { map, filter } from 'awaitly/durable';
