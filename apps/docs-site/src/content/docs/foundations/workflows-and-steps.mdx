...
Use `ErrorOf<typeof fn>` for a single dep or `Errors<[typeof fn1, typeof fn2]>` for multiple deps to derive the error union automatically. `UnexpectedError` is always included. Use `catchUnexpected` only when you want a custom unexpected type.
</Aside>
...
const result = await onboardUser.run(async ({ step, deps }) => {
  const user = await step('fetchUser', () => deps.fetchUser('1'));
