...
<CardGrid>
  ...
  <Card title="Workflows" icon="rocket">
    Compose operations with `run()` and `createWorkflow()`. Options, composition, lifecycle.

    [Learn workflows →](foundations/workflows/)
  </Card>
  <Card title="Steps" icon="puzzle">
    When to use `steps.x()` vs `step()`, plus retries, timeouts, caching, and `step.forEach`.

    [Learn steps →](foundations/step/)
  </Card>
...
flowchart LR
  subgraph creation [Creation-time options]
    createWorkflow["createWorkflow(name, deps, options) or createWorkflow(deps, options)"]
...
| `run(deps, fn)` | Compose operations once; error union inferred from deps |
| `steps.getUser(id)` | Call a dep as a step — no options, id from the dep name |
| `step('id', fn, opts)` | Same, but with retry / timeout / cache options |
