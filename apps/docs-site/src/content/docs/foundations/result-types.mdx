...
`AsyncResult<T, E>` is `Promise<Result<T, E>>`. Use it to make async function signatures cleaner.
</Aside>
...
Handle both cases in one expression. The discriminant is `type` everywhere, string errors match themselves, tagged objects match on `type`, and `TaggedError` instances expose `.type` as the canonical field (`._tag` remains as a deprecated alias).
...
**Exhaustive per-type arms**: dispatch across string literals, tagged objects, and `TaggedError` classes in one call:

```typescript
import { match, ok, err, type Result } from 'awaitly';

type FetchUserError = 'USER_NOT_FOUND' | TimeoutError | UnexpectedError;

// `await` unwraps the AsyncResult, so the value here is a `Result`.
const result: Result<User, FetchUserError> = await fetchUser('123');

const response = match(result, {
  ok: (user) => ({ status: 200, body: user }),
  USER_NOT_FOUND: () => ({ status: 404, body: { error: 'not_found' } }),
  TimeoutError: (e) => ({ status: 504, body: { error: 'timeout', ms: e.ms } }),
  UnexpectedError: (e) => ({ status: 500, body: { error: 'internal', cause: e.cause } }),
});
```

TypeScript errors if you omit a variant from the inferred union. See [Tagged Errors](foundations/tagged-errors/) for `TaggedError`-specific helpers.
...
const result = await tryAsync(
  ...
  (thrown) => ({ type: 'FETCH_FAILED', cause: thrown })
);
...
Combining operations typed as `Result<void, E>` produces `Result<void[], E>`. The ts-eslint `strict` preset rejects `void[]` through `@typescript-eslint/no-invalid-void-type`. Use `undefined` for functions that return no value when you plan to combine their Results:

```typescript
declare const revoke: (id: string) => Result<undefined, Forbidden>;

const revoked: Result<undefined[], Forbidden> = all(ids.map(revoke));
```
...
## Edge Wrapping at SDK Boundaries

Wrap SDK/vendor edges with `tryAsync` so error classification lives at the boundary, then add the `retry` policy where the function is declared so classification and retry policy stay together:
...
const authorize = () =>
  tryAsync(
    () => paymentProvider.authorize(card, total),
    (cause) =>
...
const result = await retry(authorize, {
  attempts: 4,
  delay: 100,
...
const payment = await retry(authorizePayment, {
  attempts: 3,
  delay: 100,
  backoff: 'fixed',
  retryIf: (e) => e instanceof TransientVendorError,
...
const parsed = await tryAsync(
  ...
  (cause) => new ValidationError({ field: 'input', reason: String(cause) })
);
```

## Retry with the `retry` Policy

Retry async operations without the full workflow engine. `retry` wraps any Result-returning (or plain) function and keeps its error union unchanged, if all attempts fail, the last failure propagates:
...
By default, `retry` skips `UnexpectedError` and untagged throws. Typed errors still retry. Pass `retryIf: () => true` to opt back into retrying throws. In tests, pass `createTestClock()` as `clock` so delays do not wait on real time. Policy wrappers do not pick up `run({ clock })`.

The same wrapped function drops straight into a `run()` deps object, so the policy is visible where the dependency is declared.

## Timeout with the `timeout` Policy

Bound a dependency's execution time. On timeout, resolves to `err(TimeoutError)`, adding `TimeoutError` to the error union:

```typescript
import { tryAsync, timeout } from 'awaitly';
...
// result.error may be 'FETCH_FAILED' | TimeoutError
```

Accepts milliseconds or duration strings (`'5s'`, `'500ms'`). Optional third argument `{ clock }` uses that clock for the deadline instead of wall time.

## Fallback with the `fallback` Policy

Recover from a dependency's failure. The handler receives the failure plus original arguments; base errors are consumed and only handler errors remain in the union:

```typescript
import { ok, fallback, tryAsync } from 'awaitly';
...
const result = await fallback(fetchPrimary, () => ok({ value: 'default' }))();
// On CACHE_MISS, returns ok({ value: 'default' }) — no typed errors left
```

## Composing policies

Policies nest inside-out. Timeout applies per attempt when wrapped by retry:

```typescript
import { retry, timeout } from 'awaitly';

const deps = {
  charge: retry(timeout(charge, 5000), {
    attempts: 3,
    backoff: 'exponential',
    delay: 100,
  }),
...
`awaitly-analyze` reads policy chains from the deps literal and records them on `DependencyInfo.policies` for diagrams and diagnostics.
...
| `match(result, { ok, err })` | Two-arm pattern match |
| `match(result, { ok, Variant, ... })` | Exhaustive per-type match |
...
| `retry(fn, options)` | Per-dep retry policy (also works standalone) |
| `timeout(fn, ms)` | Per-dep timeout policy; adds `TimeoutError` |
| `fallback(fn, handler)` | Per-dep fallback; consumes base errors |
...
[Learn about Workflows and Steps →](foundations/workflows/)
