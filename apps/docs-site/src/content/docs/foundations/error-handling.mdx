...
If code throws an exception (not a returned error), it becomes the string `"UNEXPECTED_ERROR"`. The original thrown value is preserved in `result.cause`:
...
<Aside type="note">
`"UNEXPECTED_ERROR"` is a plain string in the error union — not an object. The original thrown error is in `result.cause`. Use `step.try()` to convert thrown exceptions into typed errors instead.
</Aside>
...
By default, thrown exceptions become the string `"UNEXPECTED_ERROR"`. Both `run()` and `createWorkflow` include it in the error union automatically:
