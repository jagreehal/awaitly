  ...
  steps={[
    {
      id: 'callStep',
      label: 'run step',
      description: 'Return err(...) for typed errors (no throw needed).',
      duration: '1.5s',
    },
    {
      id: 'retryTimeout',
      label: 'retry and timeout',
      description: 'Retries and timeouts wrap the operation you pass to step.',
      duration: '2s',
    },
    {
      id: 'handleBoundary',
      label: 'handle at boundary',
      description: 'Map typed errors to HTTP responses, UI states, or logs.',
      duration: '1.5s',
    },
  ]}
  autoPlay={true}
...
That comment is exactly what you see on hover: `result.error` resolves to the concrete literal union (`'NOT_FOUND' | 'FETCH_ERROR' | 'EMAIL_FAILED' | UnexpectedError`), not an opaque internal alias, so a typo like `result.error === 'NOT_FUOND'` is a compile error, and `switch`/`matchError` stay exhaustive. The deps-first `run(deps, fn)` form infers and displays the same way.
...
<Aside type="note">
`UnexpectedError` is a `TaggedError` in the error union. Use `isUnexpectedError(result.error)` to narrow it, and access the original thrown error via `result.error.cause`. Use `step.try()` to convert thrown exceptions into typed errors instead.
</Aside>
...
```typescript
// Custom error type — declare the tag so it joins the error union
...
<Aside type="tip" title="Default does not retry bugs">
By default, retry skips `UnexpectedError` and untagged throws. Typed errors (including timeouts) still retry. Narrow further with `shouldRetry`. Pass `retryIf: () => true` only if you want to retry throws. Drive delays in tests with `createTestClock()`. See [Retries & Timeouts](guides/retries-timeouts/#testing-with-a-clock).
</Aside>
...
```typescript
// run() — use ErrorsOf to derive E from your deps, or specify manually
...
| `shouldRetry` | `(error) => boolean` | typed errors only | Condition for retry. Does not retry `UnexpectedError` or untagged throws. |
| `clock` | `Clock` | run/workflow clock | Time source for delays. Pass `createTestClock()` in tests. |

## Complete Example

```typescript
import { createWorkflow, TaggedError } from 'awaitly';
