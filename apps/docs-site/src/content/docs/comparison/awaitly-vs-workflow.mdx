...
Use `eslint-plugin-awaitly` to catch common mistakes. The `no-immediate-execution` and `require-thunk-for-key` rules detect when you forget the function wrapper (`step('id', fn())` instead of `step('id', () => fn())`) and autofix it for you.
</Aside>
