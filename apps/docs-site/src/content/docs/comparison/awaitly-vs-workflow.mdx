...
| ESLint Plugin | None | `eslint-plugin-awaitly` with 7 rules + autofixes |
| Runtime Visualization | Limited | `awaitly/visualize` with multiple formats |
...
| AI Agents | `DurableAgent` class | Standard AI SDK (manual tool calling) |
| Sleep | `sleep("5s")` built-in | Manual implementation |
...
export async function runRaceWorkflow() {
  return await raceWorkflow(async (step, deps) => {
...
const MODEL = ollama("granite4:latest");
...
export async function runAIWorkflow(prompt: string) {
  return await aiWorkflow(async (step) => {
    const { output } = await step(() => generateText({
      model: MODEL,
...
**Key difference**: Vercel Workflow has `DurableAgent` with built-in tool registration, streaming via `getWritable()`, and multi-turn conversation support. awaitly uses standard AI SDK which is more flexible but requires manual tool orchestration.
...
│  DurableAgent for AI              Standard AI SDK           │
