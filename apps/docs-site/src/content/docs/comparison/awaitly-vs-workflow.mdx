...
| Parallel Execution | `Promise.all()` | `step.all()` / `step.all()` with named steps; `step.map()` for arrays |
...
export async function runWorkflow(id: string) {
  return await run<User, Errors>(async ({ step }) => {
...
// createWorkflow infers errors and adds caching/retry/observability
...
Use `eslint-plugin-awaitly` to catch common mistakes. The `step-no-immediate-execution` and `step-require-thunk-for-key` rules detect when you forget the function wrapper (`step('id', fn())` instead of `step('id', () => fn())`) and autofix it for you.
</Aside>
...
export async function runParallelWorkflow(id: string) {
  return await parallelWorkflow.run(async ({ step, deps }) => {
    // Named object form (recommended - better type inference)
    const { user, posts, comments } = await step.all("Fetch all data", {
...
**Key difference**: awaitly's `step.all()` provides named steps for better observability and type inference.
...
Compare manual rollback with automatic compensation.
...
export const orderProcessingSaga = createSagaWorkflow('saga', {
  reserveInventory,
  releaseInventory,
  processPayment,
  refundPayment,
...
export async function processOrder(request: OrderRequest) {
  return await orderProcessingSaga.run(async ({ step, deps }) => {
    await step(
      "reserve-inventory",
...
export async function runAIWorkflow(prompt: string) {
  return await aiWorkflow.run(async ({ step }) => {
...
export async function simple(i: number) {
  return await simpleWorkflow.run(async ({ step, deps: { add } }) => {
...
export async function runBatchWorkflow(items: string[]) {
  return await batchWorkflow.run(async ({ step, deps: { processItem } }) => {
    // processInBatches(items, process, config, options) — progress hooks live
    // in the 4th argument, not the config.
...
- You need to run workflows anywhere (not only Vercel)
...
awaitly workflows are TypeScript. No special compiler, no platform requirements. Deploy to Vercel, AWS, GCP, your own servers, or run locally.
</Aside>
