...
**Key difference**: awaitly uses the standard AI SDK v6 API, making it compatible with any AI provider (OpenAI, Anthropic, Ollama, etc.).
...
export async function simple(i: number) {
  return await simpleWorkflow(async (step, { add }) => {
    const a = await step(add(i, 7));
    const b = await step(add(a, 8));
    return b;
  });
}
```
