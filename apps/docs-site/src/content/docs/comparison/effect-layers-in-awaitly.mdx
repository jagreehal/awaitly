...
description: How to get Effect-style dependency injection and composition with awaitly, using explicit deps and provide()
---
...
Pass whatever object matches the shape your workflow expects, whenever you want: at construction time, via `provide(...)`, at execution time, per request, or from a composition root. You can **override deps at run time** with `workflow.run(fn, { deps: { ... } })` — same workflow instance, different deps for that run (great for tests). TypeScript enforces the shape.
</Aside>
...
## `provide(...)`: Effect-Like DI Ergonomics

If you want Layer-like ergonomics, awaitly now supports `provide(...)`:

- `provide(workflow, overrides)` returns a workflow with pre-bound dep overrides.
- You can chain `.provide(...)`.
- Precedence is explicit:
  `createWorkflow deps < provide deps < run config deps`.

```typescript
import { createWorkflow } from 'awaitly/workflow';
import { provide } from 'awaitly/di';

const checkout = createWorkflow('checkout', liveDeps, {});
const checkoutWithSandboxPayments = provide(checkout, {
