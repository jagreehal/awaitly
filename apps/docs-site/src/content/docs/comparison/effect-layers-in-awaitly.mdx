...
description: How to get Effect-style dependency injection and composition with awaitly, using explicit deps and withDeps()
---

import { Tabs, TabItem } from '@astrojs/starlight/components';
import { Aside } from '@astrojs/starlight/components';

**Short answer: no.** Layers are an abstraction over something simpler: passing a typed object as a parameter. awaitly uses that primitive directly, and this page shows how.

<Aside type="tip" title="One rule">
Pass whatever object matches the shape your workflow expects, whenever you want: at construction time, via `withDeps(...)`, at execution time, per request, or from a composition root. You can **override deps at run time** with `workflow.run(fn, { deps: { ... } })`, same workflow instance, different deps for that run (great for tests). TypeScript enforces the shape.
</Aside>
...
So: **pass the typed object you want the workflow to use** (at creation, via `withDeps(...)`, and/or at run time). TypeScript enforces that it matches the interface the workflow expects. You can **override deps per run** via `workflow.run(fn, { deps: partialDeps })`, run-time merges over creation-time. Same outcomes as Effect's layers:
...
## `withDeps(...)`: Effect-Like DI Ergonomics

If you want Layer-like ergonomics, awaitly supports `withDeps(...)`:

- `withDeps(workflow, overrides)` returns a workflow with pre-bound dep overrides.
- You can chain `.withDeps(...)`.
- Precedence is explicit:
  `createWorkflow deps < withDeps deps < run config deps`.

```typescript
import { createWorkflow, withDeps } from 'awaitly';

const checkout = createWorkflow('checkout', liveDeps, {});
const checkoutWithSandboxPayments = withDeps(checkout, {
  chargeCard: sandboxChargeCard,
});

// still can override per run; run config wins
await checkoutWithSandboxPayments.run(checkoutHandler, {
  deps: { chargeCard: testDoubleChargeCard },
});
```
...
Build order is explicit in your code; no first-class Layer type, only objects and functions.
...
Effect optimizes for encoding capabilities and resources in the type system and runtime. awaitly aims for the same outcomes while staying close to normal JavaScript with minimal abstraction.
...
Effect encodes architecture in types; awaitly keeps it in values. Both give you testability and a composition root; only the mechanism differs.
...
- [Foundations: Workflows and steps](foundations/workflows/): declaring dependencies and using `step`, `deps`, and `ctx`.
