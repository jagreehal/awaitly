...
| Saga / compensation | `awaitly/workflow` (`createSagaWorkflow`) | Not a dedicated first-class module in Effect core; typically built from primitives or handled by external infrastructure |
| Durable execution | `awaitly/workflow` (`durable`) | Not a dedicated first-class module in Effect core; typically built from primitives or handled by external infrastructure |
...
```typescript
import { createWorkflow } from 'awaitly/workflow';
import { createMemoryStreamStore, getStreamReader, toAsyncIterable, collect } from 'awaitly/workflow';
...
<Tabs>
  <TabItem label="awaitly">
```typescript
import { createHITLOrchestrator, createMemoryApprovalStore, createMemoryWorkflowStateStore } from 'awaitly/workflow';
