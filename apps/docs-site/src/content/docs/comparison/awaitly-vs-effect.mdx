...
const result = await workflow(async (step) => {
  ...
  const data = await limiter.execute(() => step(() => fetchData()));
  return data;
});
```
  </TabItem>
...
const result = await workflow(async (step) => {
  ...
  const receipt = await step(chargeCard(order.total));
  await step(sendConfirmation(user.email, receipt));
...
// Option 2: Database persistence (awaitly-mongo or awaitly-postgres)
import { createMongoPersistence } from 'awaitly-mongo';
// or: import { createPostgresPersistence } from 'awaitly-postgres';
...
const workflow = createWorkflow(deps, {
  persistence: store, // Automatic state persistence
});

const result = await workflow(async (step) => {
  // Cached steps skip re-execution on resume
...
<Tabs>
  <TabItem label="awaitly">
```typescript
import { ok, err, all, allAsync } from 'awaitly';
...
// Async results
const asyncResults = await allAsync([
  fetchUser('1'),
  fetchUser('2'),
  fetchUser('3'),
]);
...
<Tabs>
  <TabItem label="awaitly">
```typescript
import { ok, err, allSettled, allSettledAsync } from 'awaitly';
...
const workflow = createWorkflow({ openConnection, query });

const result = await workflow(async (step) => {
  const conn = await step(openConnection());
  try {
    const data = await step(query(conn, 'SELECT * FROM users'));
    return data;
  } finally {
...
const result = await workflow(async (step) => {
  using conn = await step(openConnection());
  const data = await step(query(conn, 'SELECT * FROM users'));
  return data;
...
const result = pipe(
  ok(5),
  R.map(n => n * 2),
  R.andThen(n => n > 5 ? ok(n) : err('TOO_SMALL')),
  R.mapError(e => ({ code: e }))
);
...
const result = processNumber(ok(5));
```
  </TabItem>
  <TabItem label="Effect">
```typescript
...
- [Workflows in depth →](/concepts/workflows/)
