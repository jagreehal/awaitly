...
// Provide the OpenTelemetry layer, then run the Effect program.
const OtelLive = NodeSdk.layer(() => sdkConfig);
const runnable = program.pipe(Effect.provide(OtelLive));
...
<Aside type="note">
Effect lets any Effect become a span and propagates context through fibers. Its OpenTelemetry package bridges the Effect tracer, metrics, and logger through a Layer. awaitly instruments its orchestration boundaries automatically through the global OpenTelemetry API. A configured provider and a registered `ContextManager` are enough; there is no awaitly adapter or alternate runner. Custom metrics and logs still use `onEvent`, which keeps the small `run` bundle free of metric and log SDK code.
</Aside>
