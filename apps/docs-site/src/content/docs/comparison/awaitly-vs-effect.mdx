...
If you can write `async/await`, you already know awaitly. There is no `Effect.gen`, no `yield*`, no runtime. You write code that looks like the code you wrote yesterday — and you get typed errors, retries, and observability for free.
...
awaitly stays in `async/await`. Errors are inferred from your dependencies — no manual annotations, no `Effect<A, E, R>` to thread through. When you're ready for more (retries, sagas, durable execution, dependency injection), you opt in module-by-module.
...
| Dependency injection | `createWorkflow('name', deps)`, `provide(workflow, overrides)`, and override per run via `workflow.run(fn, { deps })` | Layers (Context-based DI) |
...
awaitly's step helpers are deliberately small. The canonical six cover the 80% — every other concern lives as an option on `step` itself.

| awaitly | Effect equivalent |
|---|---|
| `step('id', () => fetchUser(id))` | `yield* fetchUser(id)` (unwrap + chain) |
| `step('id', () => fn(value))` | `yield* fn(value)` (chain) |
| `if (r.ok) ... else ...` after `step` | `Effect.match(r, { onSuccess, onFailure })` |
...
<Aside type="note">
Effect has a third type parameter for dependencies (Requirements). awaitly handles dependencies through `createWorkflow`, `provide(workflow, overrides)`, and `run(fn, { deps })` overrides. Effect provides different Layers at runtime.
</Aside>
...
```text
awaitly Exponential Backoff (initialDelay: 100)
────────────────────────────────────────────
#1: 100ms   ████
#2: 200ms   ████████
#3: 400ms   ████████████████
