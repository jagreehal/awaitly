...
Where Promises stop short (they aren't "wrong", only incomplete):
...
With Promises, nothing in the function signature tells you it can fail or what it depends on. You have to read the implementation or docs. That's not "Promises are unsafe". They're incomplete for modeling application-level computation.
</Aside>
...
You can adopt awaitly incrementally. Wrap existing Promise-based code like this:
...
<Aside type="tip" title="Same syntax you already use">
awaitly uses standard async/await, so you keep Promise ergonomics and add Result type safety. There's no new syntax to learn.
</Aside>
