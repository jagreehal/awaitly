...
// Wrap a throwing function (sync: from; async: tryAsync)
...
<Aside type="note">
`Awaitly.from` executes immediately and returns a Result. neverthrow's `fromThrowable` returns a new function that you then call. For async code use `Awaitly.tryAsync`.
</Aside>
...
import { Awaitly } from 'awaitly';
const result = Awaitly.mapError(Awaitly.map(Awaitly.ok(5), n => n * 2), e => e.toUpperCase());

// Or use pipe pattern with Awaitly.R
const piped = Awaitly.pipe(
  Awaitly.ok(5),
  Awaitly.R.map(n => n * 2),
