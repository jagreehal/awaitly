...
The full enumerated list lives in `AWAITLY_SLUGS` on the root `awaitly` entry (see below). Adding a slug is non-breaking; renaming one is a major version bump.

## `AWAITLY_SLUGS` — source of truth

The slug namespace ships on the root entry. It's pure data plus helpers, so it tree-shakes out of your bundle when unused:
...
```typescript
import { type AwaitlySlug } from 'awaitly';
...
| `RateLimitError` | `runtime-rate-limit` | Wait retryAfterMs before retrying, or apply singleflight to deduplicate calls. |
...
```typescript
import { type AwaitlySystemError } from 'awaitly';
...
See the [ESLint plugin guide](guides/eslint-plugin/) for every rule.
...
<CardGrid>
  <Card title="ESLint plugin" icon="approve-check">
    [Twenty slug-named rules](guides/eslint-plugin/)
  </Card>
  <Card title="Tagged Errors" icon="warning">
    [How errors carry the spine](foundations/tagged-errors/)
  </Card>
  <Card title="Static analysis" icon="magnifier">
    [awaitly-analyze --doctor](guides/static-analysis/)
  </Card>
  <Card title="Visualization" icon="seti:html">
    [Visualizing workflow runs](guides/visualization/)
  </Card>
</CardGrid>
