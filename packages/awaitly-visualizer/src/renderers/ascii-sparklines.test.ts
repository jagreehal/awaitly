...
import { asciiRenderer, renderSparkline } from "./ascii";
import { defaultColorScheme, stripAnsi } from "./colors";
import type { EnhancedRenderOptions, WorkflowIR } from "../types";

describe("asciiRenderer sparklines", () => {
  it("renders sparklines when timingHistory is keyed by node id", () => {
    const ir: WorkflowIR = {
      root: {
        type: "workflow",
        id: "wf-1",
    ...
    const options: EnhancedRenderOptions = {
      ...
      showSparklines: true,
      timingHistory: new Map([["step-1", [5, 10, 20]]]),
    };
