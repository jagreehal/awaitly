...
function generatePathsForNodes(
  ...
  for (const node of nodes) {
    ...
    for (const state of states) {
      // Check limit before processing branching nodes, but still process all nodes
      // to complete paths (branching handlers will avoid forking when limit is hit)
      // pathCount tracks "extra paths beyond the first", so total paths = pathCount + 1
...
function generatePathsForNode(
  ...
  switch (node.type) {
    case "step":
      return handleStepNode(node, currentState, context);
...
function handleStepNode(
  ...
  // Note: pathCount is only incremented at branching points (conditionals, race)
  // to track the number of distinct paths, not steps
...
function handleRaceNode(
  ...
  // If no children, return current state unchanged
  if (node.children.length === 0) {
    return [currentState];
  }

  // Check limit BEFORE branching
  // pathCount tracks "extra paths beyond the first", so total paths = pathCount + 1
  const currentPaths = context.pathCount + 1;
  ...
  // Track pathCount before processing children (children will update it)
  ...
  for (const child of node.children) {
    // Stop early if we've already collected enough paths
    ...
    // Add only as many states as we have room for
  ...
  // Reset pathCount to before, then add our actual contribution
  // (children's updates are included in the states we return, so don't double-count)
...
function handleConditionalNode(
  ...
  // Check limit BEFORE branching - if we're at the limit, only take one branch
  // pathCount tracks "extra paths beyond the first", so total paths = pathCount + 1
  const currentPaths = context.pathCount + 1;
  const atLimit = context.hasHitLimit || currentPaths >= context.opts.maxPaths;
  if (atLimit) {
  ...
  // When limit is hit (either before or during consequent processing), don't add alternate
  // Check context.hasHitLimit which may have been set by nested branching
  ...
  // Mark limit hit if we've now reached maxPaths
...
export function calculatePathStatistics(paths: WorkflowPath[]): PathStatistics {
  ...
  return {
    ...
    pathLimitHit: false, // Would need to track this from generation
    pathsWithLoops,
