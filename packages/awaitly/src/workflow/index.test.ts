...
describe("onEvent (Phase 1 event stream)", () => {
  ...
  it("step.try emits events with name and key", async () => {
    ...
    await run<number, "ERR">(
      async (step) => {
        await step.try(() => 42, { error: "ERR", key: "try:1" });
        return 1;
      },
      {
        onError: () => {},
...
describe("createWorkflow step id/name resolution", () => {
  ...
  it("infers name from thunk when legacy form step(() => fn())", async () => {
    const events: WorkflowEvent<unknown>[] = [];
    const fetchUser = async (): AsyncResult<{ id: string }, "NOT_FOUND"> => ok({ id: "1" });
  ...
  it("infers name from thunk when step(() => deps.fn())", async () => {
    const events: WorkflowEvent<unknown>[] = [];
    const fetchUser = async (): AsyncResult<{ id: string }, "NOT_FOUND"> => ok({ id: "1" });
  ...
  it("falls back to step_N when thunk has no identifiable callee", async () => {
    const events: WorkflowEvent<unknown>[] = [];

    const workflow = createWorkflow({}, { onEvent: (e) => events.push(e) });
    ...
    const stepStart = events.find((e) => e.type === "step_start");
    expect(stepStart).toMatchObject({ type: "step_start", name: "step_1" });
    const stepSuccess = events.find((e) => e.type === "step_success");
    expect(stepSuccess).toMatchObject({ type: "step_success", name: "step_1" });
  });
});
...
describe("createWorkflow with onEvent and createContext", () => {
  ...
  it("creates fresh context for each workflow invocation", async () => {
    ...
    await workflow(async (step) => step('fetchData', () => fetchData()));
    await workflow(async (step) => step('fetchData', () => fetchData()));
...
describe("run.strict()", () => {
  ...
  it("supports onError callback", async () => {
    ...
    expect(onError).toHaveBeenCalledWith("NOT_FOUND", "directStep", undefined);
  });
...
describe("step_complete events", () => {
  ...
  it("does NOT fire step_complete for un-keyed steps", async () => {
    ...
    await workflow(async (step) => {
      // No key provided - using explicit ID
  ...
  it("fires step_complete for step.try on success", async () => {
    ...
    await workflow(async (step) => {
      return await step.try(
        ...
        { error: "PARSE_ERROR" as const, key: "parse:1" }
      );
    });
  ...
  it("fires step_complete for step.try on error", async () => {
    ...
    await workflow(async (step) => {
      return await step.try(
        ...
        { error: "PARSE_ERROR" as const, key: "parse:2" }
      );
    });
...
describe("snapshot", () => {
  it("reuses snapshot steps to avoid re-running keyed operations", async () => {
    let callCount = 0;
    const fetchUser = async (id: string): AsyncResult<{ id: string }, "ERROR"> => {
      callCount++;
      return ok({ id });
    };
  ...
  it("restores original Result.error when a step had an explicit cause", async () => {
    ...
    if (isErr(first)) {
      expect(first.error).toBe("FAILED");
    ...
    if (isErr(second)) {
      expect(second.error).toBe("FAILED");
      expect(second.cause).toBeInstanceOf(Error);
    }
  });

  it("does not throw for unused snapshot steps (could be conditional branches)", async () => {
    // We intentionally do NOT check for unknown steps after workflow completes.
    // This is because workflows can have conditional branches - a step from a skipped
    ...
    const result = await workflow2(async (step) => {
      return await step(() => ok(1), { key: "known:1" });
    });
    expect(result.ok).toBe(true);
  });
  ...
  it("emits snapshot warnings when values are not JSON-serializable", async () => {
    const workflow = createWorkflow({});
    await workflow(async (step) => {
      return await step(() => ok({ ok: true, fn: () => "nope" }), { key: "lossy:1" });
    });

    const snapshot = workflow.getSnapshot();
    expect(snapshot.warnings).toBeDefined();
    expect(snapshot.warnings?.some((warning) => warning.stepId === "lossy:1")).toBe(true);
  });
  ...
  it("warns when error value is non-JSON-serializable", async () => {
    ...
    const snapshot = workflow.getSnapshot();
    expect(snapshot.warnings?.some((warning) => warning.stepId === "lossy:error")).toBe(true);
  });
  ...
  it("clears snapshot warnings when a step becomes serializable", async () => {
    ...
    await workflow(async (step) => {
      return await step(() => ok({ fn: () => "nope" }), { key: "lossy:2" });
    });

    const withWarning = workflow.getSnapshot();
    expect(withWarning.warnings?.some((warning) => warning.stepId === "lossy:2")).toBe(true);

    await workflow(async (step) => {
      return await step(() => ok({ value: 123 }), { key: "lossy:2" });
    });

    const cleaned = workflow.getSnapshot();
    // When there are no warnings, warnings is undefined, so ?.some() returns undefined
    // The step should no longer have a warning
    expect(cleaned.warnings?.some((warning) => warning.stepId === "lossy:2") ?? false).toBe(false);
  });
  ...
  it("does not treat unused-but-defined steps as unknown", async () => {
    ...
    const result = await workflow2(async (step) => {
      ...
      if (runYesBranch) {
        return await step(() => ok("yes"), { key: "branch:yes" });
      }
      return await step(() => ok("no"), { key: "branch:no" });
    });

    expect(result.ok).toBe(true);
  });

  it("does not warn for unused snapshot steps (onUnknownSteps is no-op)", async () => {
    // onUnknownSteps cannot work correctly because we can't distinguish:
    ...
    await workflow2(async (step) => {
      return await step(() => ok(1), { key: "used:1" });
    });

    // No warning should be issued - we can't know if unused:1 is from a skipped branch
...
describe("step.sleep() method", () => {
  ...
  it("user signal cancellation emits step error event", async () => {
    ...
    const resultPromise = run(
      async (step) => {
        await step.sleep("5s", { key: "signaled-sleep", signal: controller.signal });
        return "completed";
      },
      { onEvent: (e) => events.push(e) }
    );
  ...
  it("sleep completes normally when signal is provided but never aborted", async () => {
    ...
    const result = await run(async (step) => {
      await step.sleep("50ms", { key: "normal-sleep", signal: controller.signal });
      return "completed";
    });
  ...
  it("handles invalid duration string as unexpected error", async () => {
    const result = await run(async (step) => {
      await step.sleep("invalid", { key: "bad-sleep" });
      return "completed";
    });
  ...
  it("emits step events", async () => {
    ...
    await run(
      async (step) => {
        await step.sleep("10ms", { key: "test-sleep" });
      },
      { onEvent: (e) => events.push(e) }
    );
  ...
  it("works with createWorkflow", async () => {
    ...
    const result = await workflow.run(async (step) => {
      await step.sleep("50ms", { key: "workflow-sleep" });
      return Date.now() - startTime;
    });
