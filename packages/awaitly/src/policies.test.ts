...
describe("policies", () => {
  ...
  describe("The Solution: Policies", () => {
    it("applies policy consistently across multiple steps", async () => {
      ...
      const result = await workflow.run(async ({ step, deps: { fetchUser, fetchOrders } }) => {
  ...
  describe("createPolicyRegistry", () => {
    ...
    it("works in workflow context (exact markdown example)", async () => {
      ...
      const result = await workflow.run(async ({ step, deps: { fetchUser, checkCache } }) => {
        const user = await step(
          "fetch-user",
          () => fetchUser("123"),
          policies.apply("api")
  ...
  describe("createPolicyApplier", () => {
    ...
    it("matches exact markdown example", async () => {
      ...
      const result = await workflow.run(async ({ step, deps: { fetchUser, queryOrders } }) => {
