...
/**
 * Extract node metadata for railway-style diagrams.
 *
 * Only numbers step and saga-step nodes (skipping structural nodes like
 * conditionals, parallels, etc.) to match the IDs generated by
 * renderRailwayMermaid with useNodeIds: true.
 */
export function extractRailwayNodeMetadata(ir: StaticWorkflowIR): WorkflowMetadata {
  const nodes: Record<string, NodeMetadata> = {};
  const counter = { value: 0 };

  walkRailwayNodes(ir.root.children, nodes, counter);
...
function walkRailwayNodes(
  ...
  for (const node of nodes) {
    ...
    } else if (node.type === "saga-step") {
      walkSagaStepNode(node, result, counter);
    }
    // Recurse into children of structural nodes without numbering them
    const children = getStaticChildren(node);
    if (children.length > 0) {
      walkRailwayNodes(children, result, counter);
    }
  }
}
