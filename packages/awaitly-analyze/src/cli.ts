    ...
    if (options.types) {
      for (const ir of workflows) {
        ...
        const typesFilePath = join(
          typesDir,
          `${toGeneratedTypeName(ir.root.workflowName)}.types.ts`,
        );
...
function toGeneratedTypeName(workflowName: string): string {
  ...
  if (sanitized.length === 0) return "Workflow";
 ...
 */`);
  ...
  // Output type
  const returnType = unwrapPromiseType(
    ir.root.workflowPortableReturnType ?? ir.root.workflowReturnType ?? "unknown",
  );
...
function unwrapPromiseType(type: string): string {
  const match = /^(?:Promise|PromiseLike)<([\s\S]+)>$/.exec(type.trim());
  return match?.[1] ?? type;
}
